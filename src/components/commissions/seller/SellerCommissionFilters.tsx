...
import { SellerCommissionFilters } from '@/hooks/commissions/useSellerCommissionsData';
...
export const SellerCommissionFiltersComponent: React.FC<SellerCommissionFiltersProps> = ({
  filters,
  onChange,
  isLoading = false
}) => {
  ...
  const updateFilter = (key: keyof SellerCommissionFilters, value: string | undefined) => {
  ...
  const getFilterLabel = (key: keyof SellerCommissionFilters) => {
      ...
      {hasActiveFilters && (
        <div className="flex flex-wrap gap-2">
          {Object.entries(filters).map(([key, value]) => {
            const label = getFilterLabel(key as keyof SellerCommissionFilters);
            ...
            return (
              <Badge key={key} variant="secondary" className="gap-1">
                {label}
                <button
                  onClick={() => updateFilter(key as keyof SellerCommissionFilters, undefined)}
